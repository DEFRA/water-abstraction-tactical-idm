class ReauthenticationRepository {
  ...
  async findByUserId (userId) {
    const query = `
      INSERT INTO idm.reauthentication
        (reauthentication_id, user_id, reference_date, date_created, date_updated, attempts)
        VALUES (gen_random_uuid(), $1, CURRENT_DATE, NOW(), NOW(), 0)
      ON CONFLICT (user_id, reference_date)
        DO UPDATE SET date_updated=NOW(), attempts=reauthentication.attempts+1
      RETURNING *
      `;
    const params = [userId];
